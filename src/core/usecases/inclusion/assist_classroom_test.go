...
func TestAssistClassroom_ReturnsAssistResponse(t *testing.T) {
	...
	got, err := inclusion.NewAssistClassroom(ai, students, devices, conversations, nil, nil, usage, false).
...
func TestAssistClassroom_WrapsAIError(t *testing.T) {
	...
	_, err := inclusion.NewAssistClassroom(ai, students, devices, conversations, nil, nil, usage, false).
		Execute(context.Background(), assistClassroomBaseRequest)
