...
func TestRecommendDevice(t *testing.T) {
	...
	setupMocks := func(aiResponse string, aiErr error) (*mocks.MockAIClient, *mocks.MockStudentProvider, *mocks.MockDeviceProvider, *mocks.MockRampProvider, *mocks.MockConversationProvider, *mocks.MockAIUsageProvider) {
...
[ADAPTATION_JSON:{"title":"Timer para fracciones","type":"actividad_adaptada","strategy":"Usar timer","device_ids":[1],"device_names":["Timer Visual"]}]`
		ai, students, devices, ramps, conversations, usage := setupMocks(aiResp, nil)

		got, err := inclusion.NewRecommendDevice(ai, students, devices, ramps, conversations, usage).Execute(ctx, baseRequest)
		if err != nil {
	...
	t.Run("handles response without markers", func(t *testing.T) {
		ai, students, devices, ramps, conversations, usage := setupMocks("Respuesta sin marcadores", nil)
	...
	t.Run("wraps AI error as service unavailable", func(t *testing.T) {
		ai, students, devices, ramps, conversations, usage := setupMocks("", errDB)

		_, err := inclusion.NewRecommendDevice(ai, students, devices, ramps, conversations, usage).Execute(ctx, baseRequest)
		if err == nil {
	...
	t.Run("rejects nil org_id", func(t *testing.T) {
		ai, students, devices, ramps, conversations, usage := setupMocks("", nil)
		req := baseRequest
		req.OrgID = uuid.Nil
